...
const CourseCard = ({course}) => {
    ...
    const educatorName = course.educator && typeof course.educator === 'object' ? course.educator.name : 'Instructor';
  ...
  return (
    ...
    <Link to={`/course/${course._id}`} onClick={() => scrollTo(0,0)} className='border border-gray-500/30 pb-6 overflow-hidden rounded-lg'>
        ...
        <div className='p-3 text-left'>
            ...
            <p>{calculateRating({...course, courseRatings: ratings})}</p>
            <div className='flex'>
                {[...Array(5)].map((_, i) => (<img key={i} src={i < Math.floor(calculateRating({...course, courseRatings: ratings})) ? assets.star : assets.star_blank} alt='' className='w-3.5 h-3.5' />))}
            </div>
